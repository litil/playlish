...
export const SelectedArtist: FunctionComponent<ISelectedArtistProps> = ({ artist, last }) => {
  ...
  return (
    <div className="container flex flex-row items-center mb-8">
      <div className="h-56 min-h-56 w-56 min-w-56 flex flex-row items-center">{artistImg}</div>
      <div className="flex flex-col justify-center pl-6">
        <h3 className="text-l lg:text-xl font-bold mb-2 text-left text-blue-100">{artist.name}</h3>
        {artist.tracks
          ? artist.tracks.map((track, i) => {
              return (
                <div
                  className="text-left text-customBlue-300"
